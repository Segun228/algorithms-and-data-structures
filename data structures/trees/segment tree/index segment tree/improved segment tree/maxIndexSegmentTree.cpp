...
struct Node{
    ...
    int index;
    int realIndex;
    Node() : val(INT_MIN), selfLeft(0), selfRight(0), promise(0), index(1), realIndex(0) {}
};
...
void updateElement(vector<Node>& full, int number, int newValue){
    number--;
    int tree_index, curent_index;
    for(int i=full.size()-1; i>=0; i--){
        if (full[i].selfLeft==number && full[i].selfRight==number){
            tree_index = i;
            break;
        }
    }
...
int main(){
    ...
    for(int i = 0; i<k; i++){
        ...
        if(id=='u'){
            cin >> left_input;
            cin >> right_input;
            updateElement(full, left_input, right_input);
        }
        else{
            cin >> left_input;
            cin >> right_input;
