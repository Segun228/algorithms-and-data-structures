...
class SparceTable{
    ...
    SparceTable(vector <int> nums){
        ...
        mainTable = vector<vector<int>>(height+1, vector<int>(length+1, INT_MIN));
        for(auto i=0; i<length; i++){
            this->mainTable[0][i] = nums[i];
        }
        for(int i=1, k=0; i<height+1; k+=int(pow(2, (i-1))), i++){
            for(int j=0; j<length - int(pow(2, (i-1))) - k; j++){
                mainTable[i][j] = max(mainTable[i-1][j], mainTable[i-1][j+int( pow(2, (i-1)) )]);
            }
        }
    ...
    int getMaxOnInterval(int l, int r){
        ...
        int max_val = max(mainTable[line][l], mainTable[line][r+1-int(pow(2, line))]);
        std::cout << "Maximum value from " << l << " to "<< r <<" is "<< max_val << endl;
        return max_val;
    }
...
int main(){
    ...
    table.printTable();
    table.getMaxOnInterval(0, 4);
    table.getMaxOnInterval(2, 8);
    table.getMaxOnInterval(1, 8);
    table.getMaxOnInterval(4, 5);
    table.getMaxOnInterval(3, 9);
    table.getMaxOnInterval(2, 20);
    return 0;
}